...
def test_takeoff_just_after_window_end_still_pairs(tmp_path):
    ...
    _op(conn, "l1", "landing", BASE + 100, icao24="aaa111")
    _op(conn, "t1", "takeoff", BASE + 100 + 1200, icao24="aaa111")

    # Aircraft B: lands 100s before end_ts, takes off 500s after end_ts.
    _op(conn, "l2", "landing", end_ts - 100, icao24="bbb222")
    _op(conn, "t2", "takeoff", end_ts + 500, icao24="bbb222")
    conn.commit()

    summary = dwell.dwell_summary(conn, "KLMO", start_ts, end_ts)
    assert summary == {
...
def test_based_aircraft_beyond_lookahead_horizon_still_unpaired(tmp_path):
    ...
    _op(conn, "l1", "landing", end_ts - 100, icao24="ccc333")
    _op(conn, "t1", "takeoff", end_ts + 3 * 86400, icao24="ccc333")  # 3 days later
    conn.commit()

    assert dwell.dwell_intervals(conn, "KLMO", start_ts, end_ts) == []
