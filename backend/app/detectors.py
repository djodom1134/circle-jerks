...
MIN_CIRCLE_TURN_DEGREES = 330
MIN_CIRCLE_MOVEMENT_NM = 0.03
MAX_CIRCLE_CLOSURE_NM = 1.0
...
def _detect_circles_in_samples(
    ...
    for current in sorted(samples, key=lambda row: row["timestamp"]):
        ...
        candidate_turn = cumulative_turn
        if first_course is not None:
            candidate_turn += heading_delta_deg(course, first_course)

        if max(abs(cumulative_turn), abs(candidate_turn)) < MIN_CIRCLE_TURN_DEGREES:
            continue
        turn_for_event = candidate_turn if abs(candidate_turn) > abs(cumulative_turn) else cumulative_turn
        event = circle_event(current, turn_for_event, path_nm)
        if event:
            events.append(event)
            reset(current)
...
def detect_circles(track: list[dict], airport: Airport, params: ScanParams) -> list[dict]:
    return _detect_circles_in_samples(samples_in_last(track, 20 * 60), airport, params)
...
def detect_circles_over_period(
    ...
    return _detect_circles_in_samples(samples, airport, params, start_ts, end_ts)
...
def detect_touch_and_gos_over_period(
    ...
    low_by_bucket: dict[int, list[tuple[dict, float, dict | None, float, float]]] = defaultdict(list)
    for sample in recent:
        ...
        if agl is not None and runway_dist <= 1.5 and speed is not None:
    ...
    for bucket, low_samples in sorted(low_by_bucket.items()):
        ...
        event_type = None
        if lowest_agl <= 200 and speed <= 90 and (climbed or vertical_up) and on_ground_seconds <= 60:
        ...
        events.append({
            ...
            "airport_icao": airport.icao,
            "runway_used": runway["runway_id"] if runway else None,
...
def detect_passes(track: list[dict], airport: Airport, params: ScanParams) -> list[dict]:
    ...
    inside = pass_crossing_rows(recent, airport, params, ceiling_ft=params.pass_ceiling_ft)
...
def detect_passes_over_period(
    ...
    if len(samples) < 2:
        return []
    inside = pass_crossing_rows(samples, airport, params, start_ts, end_ts, ceiling_ft=params.pass_ceiling_ft)
